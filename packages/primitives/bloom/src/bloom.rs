...
mod test {
    ...
    fn test_add_contains() {
        ...
        let key = kp_core::keccak_256(b"hello").into();
        assert!(!bloom.contains(&key));
        bloom.add(&key);
        assert!(bloom.contains(&key));

        let key = kp_core::keccak_256(b"world").into();
        assert!(!bloom.contains(&key));
        bloom.add(&key);
        assert!(bloom.contains(&key));
    }
    ...
    fn test_debug() {
        ...
        b.add(&Hash::default());
        b.add(&kp_core::keccak_256(&[1, 2]).into());
        assert_eq!(
            format!("{:?}", b),
    ...
    fn test_atomic_bloom() {
        ...
        let hash_values: Vec<_> = std::iter::repeat_with(|| Hash::random_using(&mut rng))
            .take(1200)
            .collect();
        ...
        let false_positive = std::iter::repeat_with(|| Hash::random_using(&mut rng))
            .take(10_000)
    ...
    fn test_atomic_bloom_round_trip() {
        ...
        let more_hash_values: Vec<_> =
            std::iter::repeat_with(|| Hash::random_using(&mut rng))
                .take(1000)
                .collect();
        let bloom: AtomicBloom<_> = bloom.into();
        assert_eq!(bloom.num_bits, 9731);
