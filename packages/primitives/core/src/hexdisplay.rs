...
impl<'a> kp_std::fmt::Display for HexDisplay<'a> {
	fn fmt(&self, f: &mut kp_std::fmt::Formatter) -> Result<(), kp_std::fmt::Error> {
		if self.0.len() < 1027 {
			for byte in self.0 {
...
impl<'a> kp_std::fmt::Debug for HexDisplay<'a> {
	fn fmt(&self, f: &mut kp_std::fmt::Formatter) -> Result<(), kp_std::fmt::Error> {
		for byte in self.0 {
			f.write_fmt(format_args!("{:02x}", byte))?;
		}
...
impl AsBytesRef for kp_std::vec::Vec<u8> {
	fn as_bytes_ref(&self) -> &[u8] {
		self
	}
}
