...
#[doc(hidden)]
pub use kp_std::ops::Deref;
use kp_std::{hash::Hash, str, vec::Vec};
	...
	pub fn soft<T: Encode>(index: T) -> Self {
		...
		index.using_encoded(|data| {
			if data.len() > JUNCTION_ID_LEN {
				cc.copy_from_slice(&kp_core_hashing::blake2_256(data));
			} else {
...
#[cfg(feature = "std")]
impl kp_std::fmt::Debug for PublicError {
...
impl kp_std::fmt::Debug for AccountId32 {
	#[cfg(feature = "std")]
	...
	#[cfg(not(feature = "std"))]
	fn fmt(&self, _: &mut kp_std::fmt::Formatter) -> kp_std::fmt::Result {
		Ok(())
	}
}
...
#[cfg(feature = "std")]
impl kp_std::str::FromStr for AccountId32 {
	type Err = &'static str;
	...
	impl AsMut<[u8]> for Dummy {
		fn as_mut(&mut self) -> &mut [u8] {
			unsafe {
				...
				kp_std::mem::transmute::<_, &'static mut [u8]>(&b""[..])
			}
		}
	}
...
#[cfg(feature = "std")]
impl kp_std::str::FromStr for SecretUri {
...
#[cfg(feature = "std")]
impl kp_std::fmt::Display for CryptoTypePublicPair {
	fn fmt(&self, f: &mut kp_std::fmt::Formatter) -> kp_std::fmt::Result {
