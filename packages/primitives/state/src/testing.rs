...
use kp_externalities::{Extension, ExtensionStore, Extensions};
use kp_trie::StorageProof;
	...
	H::Out: Ord + 'static + codec::Codec,
{
	...
	pub fn insert_child(&mut self, c: kp_core::storage::ChildInfo, k: StorageKey, v: StorageValue) {
	...
	pub fn execute_with<R>(&mut self, execute: impl FnOnce() -> R) -> R {
		...
		kp_externalities::set_and_run_with_externalities(&mut ext, execute)
	}
	...
	pub fn execute_and_prove<R>(&mut self, execute: impl FnOnce() -> R) -> (R, StorageProof) {
		...
		let outcome = kp_externalities::set_and_run_with_externalities(&mut proving_ext, execute);
	...
	pub fn execute_with_safe<R>(
		...
		std::panic::catch_unwind(move || {
			kp_externalities::set_and_run_with_externalities(&mut *ext, f)
		})
...
impl<H> kp_externalities::ExtensionStore for TestExternalities<H>
where
	H: Hasher,
	H::Out: Ord + codec::Codec,
{
	...
	fn deregister_extension_by_type_id(
		...
		if self.extensions.deregister(type_id) {
			...
			Err(kp_externalities::Error::ExtensionIsNotRegistered(type_id))
		}
	}
}

impl<H> kp_externalities::ExternalitiesExt for TestExternalities<H>
where
	H: Hasher,
	H::Out: Ord + codec::Codec,
{
	...
	fn register_extension<T: Extension>(&mut self, ext: T) -> Result<(), kp_externalities::Error> {
		self.register_extension_with_type_id(TypeId::of::<T>(), Box::new(ext))
	}

	fn deregister_extension<T: Extension>(&mut self) -> Result<(), kp_externalities::Error> {
...
mod tests {
	use super::*;
	use kp_core::{storage::ChildInfo, traits::Externalities, H256};
	use kp_core::Blake2Hasher as BlakeTwo256;
