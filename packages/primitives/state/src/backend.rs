...
pub trait Backend<H: Hasher>: kp_std::fmt::Debug {
...
#[cfg(feature = "std")]
pub trait AsTrieBackend<H: Hasher, C = kp_trie::cache::LocalTrieCache<H>> {
	/// Type of trie backend storage.
...
impl<H, KF> Consolidate for kp_trie::GenericMemoryDB<H, KF>
where
	H: Hasher,
	KF: kp_trie::KeyFunction<H>,
{
	fn consolidate(&mut self, other: Self) {
		kp_trie::GenericMemoryDB::consolidate(self, other)
	}
}
...
#[cfg(feature = "std")]
impl<'a, B: Backend<H>, H: Hasher> kp_core::traits::FetchRuntimeCode
	for BackendRuntimeCode<'a, B, H>
{
	fn fetch_runtime_code(&self) -> Option<std::borrow::Cow<[u8]>> {
		self.backend
			.storage(kp_core::storage::well_known_keys::CODE)
			.ok()
			.flatten()
			.map(Into::into)
	}
	...
	H::Out: Encode,
{
	...
	pub fn runtime_code(&self) -> Result<RuntimeCode, &'static str> {
		let hash = self
			.backend
			.storage_hash(kp_core::storage::well_known_keys::CODE)
			.ok()
			.flatten()
		...
		let heap_pages = self
			.backend
			.storage(kp_core::storage::well_known_keys::HEAP_PAGES)
			.ok()
			.flatten()
