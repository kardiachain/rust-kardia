...
use kp_externalities::{Extension, Extensions, MultiRemovalResults};
use kp_trie::{empty_child_trie_root, HashKey, LayoutV0, LayoutV1, TrieConfiguration};
use std::{
	any::{Any, TypeId},
...
impl BasicExternalities {
	...
	pub fn execute_with_storage<R>(
		storage: &mut kp_core::storage::Storage,
		f: impl FnOnce() -> R,
	) -> R {
	...
	pub fn execute_with<R>(&mut self, f: impl FnOnce() -> R) -> R {
		kp_externalities::set_and_run_with_externalities(self, f)
	}
...
impl kp_externalities::ExtensionStore for BasicExternalities {
	...
	fn register_extension_with_type_id(
		&mut self,
		type_id: TypeId,
		extension: Box<dyn kp_externalities::Extension>,
