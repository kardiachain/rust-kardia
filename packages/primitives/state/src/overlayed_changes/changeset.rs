...
#[cfg(not(feature = "std"))]
use kp_std::collections::btree_set::BTreeSet as Set;
#[cfg(feature = "std")]
...
#[cfg(feature = "std")]
impl From<kp_core::storage::StorageMap> for OverlayedMap<StorageKey, Option<StorageValue>> {
	fn from(storage: kp_core::storage::StorageMap) -> Self {
		Self {
			changes: storage
				.into_iter()
				.map(|(k, v)| {
