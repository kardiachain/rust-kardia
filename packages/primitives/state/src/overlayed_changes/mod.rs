...
#[cfg(feature = "std")]
use kp_externalities::{Extension, Extensions};
#[cfg(not(feature = "std"))]
use kp_std::collections::btree_map::BTreeMap as Map;
use kp_std::{collections::btree_set::BTreeSet, vec::Vec};
#[cfg(feature = "std")]
...
/// In memory array of storage values.
pub type OffchainChangesCollection = Vec<(Vec<u8>, Vec<u8>)>;
...
#[cfg(feature = "std")]
impl From<kp_core::storage::Storage> for OverlayedChanges {
	fn from(storage: kp_core::storage::Storage) -> Self {
		Self {
...
where
	...
	F: FnMut(&K, &mut V) -> bool,
{
	let old = kp_std::mem::replace(map, Map::default());
...
impl<'a> OverlayedExtensions<'a> {
	...
	pub fn register(
		...
		match self.extensions.entry(type_id) {
			...
			MapEntry::Occupied(_) => Err(kp_externalities::Error::ExtensionAlreadyRegistered),
		}
	}
...
mod tests {
	...
	use kp_core::{traits::Externalities, Blake2Hasher};
