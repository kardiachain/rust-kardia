...
use codec::Codec;
use hash_db::Hasher;
use kp_core::storage::{ChildInfo, StateVersion, Storage};
use kp_trie::{empty_trie_root, GenericMemoryDB, HashKey, KeyFunction, LayoutV1, MemoryDB};
	...
	KF: KeyFunction<H> + Send + Sync,
{
	...
	pub fn apply_transaction(&mut self, root: H::Out, transaction: GenericMemoryDB<H, KF>) {
		let mut storage = kp_std::mem::take(self).into_storage();
...
mod tests {
	...
	use kp_core::storage::StateVersion;
