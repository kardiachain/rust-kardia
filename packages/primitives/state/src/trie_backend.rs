...
use hash_db::HashDB;
use hash_db::Hasher;
use kp_core::storage::{ChildInfo, StateVersion};
use kp_std::vec::Vec;
#[cfg(feature = "std")]
use kp_trie::{cache::LocalTrieCache, recorder::Recorder};
#[cfg(feature = "std")]
use kp_trie::{MemoryDB, StorageProof};

/// Dummy type to be used in `no_std`.
///
/// This is required to have the type available for [`TrieBackendBuilder`] and [`TrieBackend`].
#[cfg(not(feature = "std"))]
pub struct LocalTrieCache<H>(kp_std::marker::PhantomData<H>);
...
impl<S: TrieBackendStorage<H>, H: Hasher, C: AsLocalTrieCache<H>> kp_std::fmt::Debug
	for TrieBackend<S, H, C>
{
	fn fmt(&self, f: &mut kp_std::fmt::Formatter<'_>) -> kp_std::fmt::Result {
		write!(f, "TrieBackend")
	}
}
...
pub mod tests {
	...
	type Recorder = kp_trie::recorder::Recorder<BlakeTwo256>;
	...
	pub(crate) fn test_db(state_version: StateVersion) -> (PrefixedMemoryDB<BlakeTwo256>, H256) {
...

		{
			...
			fn build<L: kp_trie::TrieLayout>(
				mut trie: kp_trie::TrieDBMut<L>,
				child_info: &ChildInfo,
				sub_root: &[u8],
			) {
	...
	fn child_proof_recording_with_edge_cases_works_inner(state_version: StateVersion) {
		...
		let nodes = {
			...
			for node in proof.iter_nodes() {
				...
				if hash != value_hash {
					let node = kp_trie::NodeCodec::<BlakeTwo256>::decode(&node)
						.unwrap()
						.to_owned_node::<kp_trie::LayoutV1<BlakeTwo256>>()
						.unwrap();
