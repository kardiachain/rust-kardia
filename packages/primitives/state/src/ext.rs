...
use kp_externalities::{Extension, ExtensionStore, Externalities, MultiRemovalResults};
use kp_trie::{empty_child_trie_root, LayoutV1};
...
#[cfg(feature = "std")]
fn guard() -> kp_panic_handler::AbortGuard {
	kp_panic_handler::AbortGuard::force_abort()
}
	...
	B: Backend<H>,
{
	...
	pub fn new(
		...
		storage_transaction_cache: &'a mut StorageTransactionCache<B::Transaction, H>,
		backend: &'a B,
		extensions: Option<&'a mut kp_externalities::Extensions>,
	) -> Self {
		Self {
			overlay,
			backend,
	...
	B: Backend<H>,
{
	...
	fn clear_prefix(
		...
		if kp_core::storage::well_known_keys::starts_with_child_storage_key(prefix) {
			warn!(
				target: "trie",
...
impl<'a> StorageAppend<'a> {
	...
	pub fn append(&mut self, value: Vec<u8>) {
		...
		let item = kp_std::mem::take(self.0);
	...
	B: Backend<H>,
{
	...
	fn register_extension_with_type_id(
		...
		Err(kp_externalities::Error::ExtensionsAreNotSupported)
	}
...
mod tests {
	...
	fn clear_prefix_cannot_delete_a_child_root() {
		...
		use kp_core::storage::well_known_keys;
		let mut ext = ext;
