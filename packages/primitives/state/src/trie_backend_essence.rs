...
use kp_core::storage::{ChildInfo, ChildType, StateVersion};
#[cfg(not(feature = "std"))]
use kp_std::marker::PhantomData;
use kp_std::{boxed::Box, vec::Vec};
#[cfg(feature = "std")]
use kp_trie::recorder::Recorder;
use kp_trie::{
...
use kp_trie::LayoutV1 as Layout;
...
type Result<V> = kp_std::result::Result<V, crate::DefaultError>;
	...
	H::Out: Codec + Ord,
{
	...
	fn next_storage_key_from_root(
		...
		self.with_recorder_and_cache(Some(*root), |recorder, cache| {
			...
			// * and no keys can be inserted between `key` and `key++0` (this is ensured by kp-io).
	...
	fn trie_iter_key_inner<F: FnMut(&[u8]) -> bool>(
		...
		let mut iter = move |db| -> kp_std::result::Result<(), Box<TrieError<H::Out>>> {
			self.with_recorder_and_cache(Some(*root), |recorder, cache| {
	...
	fn trie_iter_inner<F: FnMut(Vec<u8>, Vec<u8>) -> bool>(
		...
		let mut iter = move |db| -> kp_std::result::Result<bool, Box<TrieError<H::Out>>> {
			self.with_recorder_and_cache(Some(*root), |recorder, cache| {
				let trie = TrieDBBuilder::<H>::new(db, root)
					.with_optional_recorder(recorder)
					.with_optional_cache(cache)
	...
	pub fn pairs(&self) -> Vec<(StorageKey, StorageValue)> {
		let collect_all = || -> kp_std::result::Result<_, Box<TrieError<H::Out>>> {
			self.with_recorder_and_cache(None, |recorder, cache| {
	...
	pub fn storage_root<'a>(
		...
		let root = self.with_recorder_and_cache_for_storage_root(|recorder, cache| {
			...
			let res = match state_version {
				StateVersion::V0 => delta_trie_root::<kp_trie::LayoutV0<H>, _, _, _, _, _>(
					&mut eph, self.root, delta, recorder, cache,
				),
				StateVersion::V1 => delta_trie_root::<kp_trie::LayoutV1<H>, _, _, _, _, _>(
					&mut eph, self.root, delta, recorder, cache,
				),
			};
	...
	pub fn child_storage_root<'a>(
		...
		let default_root = match child_info.child_type() {
			ChildType::ParentKeyId => empty_child_trie_root::<kp_trie::LayoutV1<H>>(),
		};
		let mut write_overlay = S::Overlay::default();
		let child_root = match self.child_root(child_info) {
			Ok(Some(hash)) => hash,
		...
		let new_child_root = self.with_recorder_and_cache_for_storage_root(|recorder, cache| {
			...
			match match state_version {
				StateVersion::V0 =>
					child_delta_trie_root::<kp_trie::LayoutV0<H>, _, _, _, _, _, _>(
						child_info.keyspace(),
						&mut eph,
						child_root,
						delta,
				...
				StateVersion::V1 =>
					child_delta_trie_root::<kp_trie::LayoutV1<H>, _, _, _, _, _, _>(
						child_info.keyspace(),
						&mut eph,
						child_root,
						delta,
...
impl<H: Hasher> TrieBackendStorage<H> for Arc<dyn Storage<H>> {
	type Overlay = kp_trie::PrefixedMemoryDB<H>;
...
impl<H, KF> TrieBackendStorage<H> for kp_trie::GenericMemoryDB<H, KF>
where
	H: Hasher,
	KF: kp_trie::KeyFunction<H> + Send + Sync,
{
	type Overlay = Self;
