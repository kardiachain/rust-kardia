...
pub(crate) fn size_and_prefix_iterator(
	...
	let size = kp_std::cmp::min(trie_constants::NIBBLE_SIZE_BOUND, size);

	let max_value = 255u8 >> prefix_mask;
	let l1 = kp_std::cmp::min((max_value as usize).saturating_sub(1), size);
	...
	first_byte.chain(kp_std::iter::from_fn(next_bytes))
}
