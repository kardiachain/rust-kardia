	...
	H: Hasher,
{
	const USE_EXTENSION: bool = false;
	const ALLOW_EMPTY: bool = true;
	const MAX_INLINE_VALUE: Option<u32> = Some(kp_core::storage::TRIE_VALUE_NODE_THRESHOLD);
...
fn keyspace_as_prefix_alloc(ks: &[u8], prefix: Prefix) -> (Vec<u8>, Option<u8>) {
	let mut result = kp_std::vec![0; ks.len() + prefix.0.len()];
...
mod tests {
	...
	fn generate_storage_root_with_proof_works_independently_from_the_delta_order() {
		...
		let storage_root =
			kp_core::H256::decode(&mut &include_bytes!("../test-res/storage_root")[..]).unwrap();
