...
impl<'a, H: Hasher> trie_db::TrieCache<NodeCodec<H>> for TrieCache<'a, H> {
	...
	fn lookup_value_for_key(&mut self, key: &[u8]) -> Option<&CachedValue<H::Out>> {
		...
		tracing::trace!(
			target: LOG_TARGET,
			key = ?kp_core::hexdisplay::HexDisplay::from(&key),
			found = res.is_some(),
...
mod tests {
	...
	type MemoryDB = crate::MemoryDB<kp_core::Blake2Hasher>;
	type Layout = crate::LayoutV1<kp_core::Blake2Hasher>;
	type Cache = super::SharedTrieCache<kp_core::Blake2Hasher>;
	type Recorder = crate::recorder::Recorder<kp_core::Blake2Hasher>;
